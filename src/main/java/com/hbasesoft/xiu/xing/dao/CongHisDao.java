package com.hbasesoft.xiu.xing.dao;

import com.hbasesoft.framework.db.Dao;
import com.hbasesoft.framework.db.core.annotation.Param;
import com.hbasesoft.framework.db.core.annotation.Sql;
import com.hbasesoft.framework.db.hibernate.IGenericBaseDao;
import com.hbasesoft.xiu.xing.entity.CongHisEntity;

/**
 * <Description> <br>
 *
 * @author 付永杰<br>
 * @version 1.0<br>
 * @taskId <br>
 * @CreateDate 2020年07月18日 <br>
 * @see com.hbasesoft.xiu.xing.dao <br>
 * @since V1.0 <br>
 */
@Dao
public interface CongHisDao extends IGenericBaseDao {

    @Sql("update cong_his set cong_id = :entity.congId,cong_code = :entity.congCode where id = :entity.id")
    int updateCongHisById(@Param("entity") CongHisEntity entity);
}
